...
import { isAdmin, isSupervisor, isStaffRole } from './lib/permissions';
...
function App() {
  ...
  const userIsAdmin = isAdmin(profileRole);
  const userIsSupervisor = isSupervisor(profileRole);
  const isStaff = isStaffRole(profileRole);
  ...
  const navItems = useMemo(() => {
    ...
    baseItems.push({
      key: 'profile',
      label: userIsAdmin ? 'My Admin Profile' : 'My QA Profile',
    });

    return baseItems;
  }, [profile, isStaff, userIsAdmin]);
  ...
  function renderStaffPage() {
    switch (page) {
      ...
      case 'accounts':
        return userIsAdmin ? <AccountsSupabase /> : null;
      case 'supervisorRequests':
        return userIsAdmin && profile ? (
        ...
        return (
          <div style={styles.profilePanel}>
            ...
            <h2 style={{ marginTop: 0, marginBottom: '18px', color: theme.brandTitle }}>
              {userIsAdmin ? 'My Admin Profile' : 'My QA Profile'}
            </h2>
