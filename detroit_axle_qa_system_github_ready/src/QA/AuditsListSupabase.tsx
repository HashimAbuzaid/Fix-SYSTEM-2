...
/* ═════════════════════════════════════════════════════════════
   FIX #1 — Theme: inject CSS vars once via <style> tag.
   The original approach spread 50+ vars as inline styles on every
   render, forcing a full style-recalculation on the entire subtree.
   Now we write them once into the document head and never touch them
   again from JS during normal interaction.
   ═════════════════════════════════════════════════════════════ */

const AL_STYLE_ID = 'da-audits-list-theme-v1';

function buildAuditsThemeCss(isLight: boolean): string {
  /* All 50 tokens in one CSS rule — written once, swapped on theme toggle */
  return `
:root {
...
/**
 * FIX #1 — Inject theme CSS vars into a single <style> tag and update
 * it only when the theme attribute changes. No React state, no re-renders.
 */
function useAuditsThemeInjection() {
  useEffect(() => {
    ...
    return () => {
      obs.disconnect();
      document.getElementById(AL_STYLE_ID)?.remove();
    };
  }, []);
}
...
/* ═════════════════════════════════════════════════════════════
   Loader Visual (stable, never changes)
   ═════════════════════════════════════════════════════════════ */
...
function AuditsListSupabase() {
  // FIX #1 — inject theme CSS once, no re-renders on theme change
  ...
  /* FIX #2 — useDeferredValue stays but we make sure memos that use it
     don't also take unstable object references as deps */
  ...
  /* ── FIX #3 — Stable lookup maps via useMemo ──────────────────────
     Original code called getDisplayName() inside filteredAudits memo
     but defined it as a plain function closing over profiles, making
     the dep array unstable. Now we memoise the Map once. */
  ...
  /* FIX #3 cont. — stable helper functions using the memoised maps */
  const getDisplayName = useCallback(
    (audit: AuditItem) => displayNameByKey.get(agentKey(audit.agent_id, audit.team)) || null,
    [displayNameByKey]
  );
  const getCreatedByLabel = useCallback(
    (audit: AuditItem) => audit.created_by_name || audit.created_by_email || '—',
    []
  );
  const getAuditReference = useCallback((audit: AuditItem) => {
    if (audit.team === 'Tickets') return `Ticket: ${audit.ticket_id || '—'}`;
    return `Order #${audit.order_number || '—'} · ${audit.phone_number || '—'}`;
  }, []);
  const getCommentsPreview = useCallback((v?: string | null) => {
    const t = (v || '').trim();
    if (!t) return '—';
    return t.length <= 120 ? t : `${t.slice(0, 117)}…`;
  }, []);
  const getAgentLabel = useCallback((p: AgentProfile) =>
    p.display_name ? `${p.agent_name} — ${p.display_name}` : `${p.agent_name} · ${p.agent_id}`, []);
  ...
  /* FIX #4 — filteredAudits uses the stable getDisplayName from useCallback */
  const filteredAudits = useMemo(() => {
    ...
    return audits.filter((a) => {
      ...
      if (caseTypeFilter && a.case_type !== caseTypeFilter) return false;
      if (dateFrom && a.audit_date < dateFrom) return false;
      if (dateTo && a.audit_date > dateTo) return false;
      return true;
    });
  }, [audits, deferredSearch, displayNameByKey, teamFilter, caseTypeFilter, dateFrom, dateTo]);
  ...
  const getEffectiveOffIndexes = useCallback(
    (aid?: string | null, team?: string | null) => {
  ...
  /* ── FIX #5 — progressData: move display-name lookup inside so the
     memo only depends on the stable Map, not on a function reference ── */
  const progressData = useMemo(() => {
    ...
    filteredAudits.forEach((a) => {
      ...
      const dn = displayNameByKey.get(k) || null;  // use stable Map directly
    ...
    scopedProfiles.forEach((p) => {
      ...
      if (!grouped.has(k))
        grouped.set(k, { agent_id: p.agent_id, agent_name: p.agent_name, display_name: p.display_name, team: p.team, evaluations: [] });
    });
  ...
  const toggleAgentOff = useCallback(
    async (aid?: string | null, team?: AuditItem['team'] | null) => {
      ...
      try {
        ...
        setErrorMessage(e instanceof Error ? e.message : 'Could not update OFF markers.');
      }
    },
  ...
  const getResultColor = useCallback((r: string): string => {
  ...
  /* FIX #1 cont. — root div no longer receives 50 inline style vars */
  return (
    <div style={S.root}>
      {/* Page Header */}
            ...
            <div style={S.tableWrap}>
              <div style={{ minWidth: '2600px' }}>
                ...
                <div style={{ ...S.progressRow, gridTemplateColumns: gridTemplate, background: 'var(--al-surface-soft)', borderBottom: '1px solid var(--al-border)', position: 'sticky', top: 0, zIndex: 2 }}>
                  ...
                  {visibleCols.map((col) => (
                    <button key={col.label} onClick={() => setSelectedOffEvalIndexes((prev) => prev.includes(col.index) ? normalizeOffEvalIndexes(prev.filter((i) => i !== col.index)) : normalizeOffEvalIndexes([...prev, col.index]))}
                ...
                {/* FIX #6 — each progress row is a memoised component so only
                    changed rows re-render when selectedOffEvalIndexes changes */}
                {progressData.rows.map((row) => (
                  <ProgressRow
                    ...
                    onToggleOff={toggleAgentOff}
                    formatOffSummary={formatOffSummary}
                  />
                ))}
              </div>
            </div>
        ...
        <div style={{ ...S.tableWrap, marginTop: '14px' }}>
          <div style={{ minWidth: '1800px' }}>
            ...
            {filteredAudits.map((audit) => (
              /* FIX #6 cont. — memoised row; re-renders only when this specific
                 audit changes, not when any other state updates in parent */
              <AuditTableRow
                key={audit.id}
                audit={audit}
                isAdmin={isAdmin}
...
/* ═════════════════════════════════════════════════════════════
   FIX #6 — ProgressRow: fully memoised progress board row.
   In the original code every row re-rendered whenever the parent
   typed a character into the search box or toggled an eval column.
   Now only the affected row re-renders.
   ═════════════════════════════════════════════════════════════ */
...
const ProgressRow = memo(function ProgressRow({
  row, visibleCols, gridTemplate, selectedOffEvalIndexes, canManageOffToday, onToggleOff, formatOffSummary,
}: {
  row: ProgressRowData;
  visibleCols: EvalCol[];
  gridTemplate: string;
  ...
  onToggleOff: (id?: string | null, team?: AuditItem['team'] | null) => Promise<void>;
  ...
  return (
    <div style={{ ...S.progressRow, gridTemplateColumns: gridTemplate, borderBottom: '1px solid var(--al-border-faint)', padding: '10px 16px' }}>
      ...
      {visibleCols.map((col) => {
        ...
        if (isOff)
          return <div key={`${row.agent_id}-${col.index}`} style={{ ...S.evalCell, ...S.evalOff, ...(isSel ? S.evalSelected : {}) }} title={`OFF – ${col.label}`}>OFF</div>;
...
/* ═════════════════════════════════════════════════════════════
   FIX #6 cont. — AuditTableRow: memoised so only touched rows
   re-render. Prop equality check ensures stable callbacks don't
   break memoisation.
   ═════════════════════════════════════════════════════════════ */

const AuditTableRow = memo(function AuditTableRow({
  audit, isAdmin, isEditing, isExpanded, releaseLoadingId, saving, bulkSaving,
  onExpandToggle, onEdit, onShare, onDelete,
  getDisplayName, getCreatedByLabel, getAuditReference, getCommentsPreview, children,
}: {
  ...
  return (
    <div style={S.auditEntry}>
      <div style={S.auditRow}>
        ...
        <div>
          <span style={{ ...S.scoreBadge, ...(band === 'strong' ? S.evalStrong : band === 'medium' ? S.evalMedium : band === 'weak' ? S.evalWeak : S.evalEmpty) }}>
            {score.toFixed(2)}%
          </span>
        </div>
        <div>
        ...
        <div style={S.actionCell}>
          ...
          {isAdmin && (
            <>
              <button onClick={onShare} disabled={releaseLoadingId === audit.id || saving || bulkSaving}
                style={audit.shared_with_agent ? S.btnMiniDanger : S.btnMiniPrimary}>
...
/* ═════════════════════════════════════════════════════════════
   EditAuditForm & AuditDetailView — unchanged logic, kept separate
   ═════════════════════════════════════════════════════════════ */

function EditAuditForm({
  editForm, setEditForm, editScores, editMetricComments, setEditMetricComments,
  selectedAgentProfileId, setSelectedAgentProfileId, agentSearch, setAgentSearch,
  isAgentPickerOpen, setIsAgentPickerOpen, agentPickerRef, visibleAgents, selectedAgent,
  getAgentLabel, onTeamChange, onScoreChange, onUpdate, onCancel, saving,
}: {
  ...
  visibleAgents: AgentProfile[]; selectedAgent: AgentProfile | null;
  getAgentLabel: (p: AgentProfile) => string;
  onTeamChange: (t: EditFormState['team']) => void; onScoreChange: (name: string, val: string) => void;
  onUpdate: () => void; onCancel: () => void; saving: boolean;
}) {
  ...
  return (
    <div style={S.expandedPanel}>
      ...
      <div style={S.editGrid}>
              ...
              <div style={S.pickerMenu}>
                ...
                <div style={{ maxHeight: '260px', overflowY: 'auto', padding: '8px', display: 'grid', gap: '6px' }}>
                  {visibleAgents.length === 0 ? (
                    <div style={{ padding: '12px', color: 'var(--al-subtle)' }}>No agents found</div>
                  ) : visibleAgents.map((p) => (
        ...
        <div>
          <label style={S.label}>Case Type</label>
          <input value={editForm.caseType} onChange={(e) => setEditForm((prev) => ({ ...prev, caseType: e.target.value }))} style={S.input} />
        </div>
        <div>
          <label style={S.label}>Audit Date</label>
          <input type="date" value={editForm.auditDate} onChange={(e) => setEditForm((prev) => ({ ...prev, auditDate: e.target.value }))} onClick={(e) => openNativeDatePicker(e.currentTarget)} style={S.input} />
        </div>
        {(editForm.team === 'Calls' || editForm.team === 'Sales') && (
          <>
            <div>
              <label style={S.label}>Order Number</label>
              <input value={editForm.orderNumber} onChange={(e) => setEditForm((prev) => ({ ...prev, orderNumber: e.target.value }))} style={S.input} />
            </div>
            <div>
              <label style={S.label}>Phone Number</label>
              <input value={editForm.phoneNumber} onChange={(e) => setEditForm((prev) => ({ ...prev, phoneNumber: e.target.value }))} style={S.input} />
            </div>
          </>
        )}
        {editForm.team === 'Tickets' && (
          <div>
            ...
            <input value={editForm.ticketId} onChange={(e) => setEditForm((prev) => ({ ...prev, ticketId: e.target.value }))} style={S.input} />
          </div>
        )}
        <div style={{ gridColumn: '1/-1' }}>
          <label style={S.label}>Comments</label>
          <textarea value={editForm.comments} onChange={(e) => setEditForm((prev) => ({ ...prev, comments: e.target.value }))} rows={3} style={{ ...S.input, resize: 'vertical' }} />
        </div>
      </div>
          ...
          return (
            <div key={m.name} style={S.metricRow}>
              <div style={{ flex: 1 }}>
                ...
                {showComment && (
                  <textarea value={editMetricComments[m.name] || ''} onChange={(e) => setEditMetricComments((prev) => ({ ...prev, [m.name]: e.target.value }))}
                    rows={2} placeholder="Short QA note for this result…" style={{ ...S.input, marginTop: '8px', resize: 'vertical', fontSize: '12px' }} />
                )}
              </div>
  ...
  return (
    <div style={S.expandedPanel}>
      ...
      <div style={{ ...S.detailCard, marginBottom: '18px' }}>
        ...
        <div style={{ color: 'var(--al-text)', fontSize: '14px', lineHeight: 1.7, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
          {audit.comments?.trim() || '—'}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gap: '8px' }}>
        {(audit.score_details || []).map((d) => (
          <div key={`${audit.id}-${d.metric}`} style={S.scoreDetailRow}>
            ...
            <span style={{ ...S.resultBadge, background: getResultColor(d.result) + '22', color: getResultColor(d.result), borderColor: getResultColor(d.result) + '44' }}>
              {d.result}
            </span>
          </div>
        ))}
...
/* ═════════════════════════════════════════════════════════════
   Styles (module scope — stable object references, never recreated)
   ═════════════════════════════════════════════════════════════ */
