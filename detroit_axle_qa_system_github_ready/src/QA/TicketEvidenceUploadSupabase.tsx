...
const emptyDraft: TicketEvidenceDraft = {
...
function TicketEvidenceUploadSupabase() {
  const [draft, setDraft] = usePersistentState<TicketEvidenceDraft>(
    'detroit-axle-ticket-evidence-draft',
    emptyDraft
  );
  ...
  const [preparedCsvRows, setPreparedCsvRows] = useState<PreparedEvidenceRow[]>([]);
  const [skippedCsvRows, setSkippedCsvRows] = useState<SkippedEvidenceRow[]>([]);
  ...
  async function handleCsvFileChange(file?: File | null) {
    ...
    try {
      ...
      if (rows.length === 0) {
        setErrorMessage('The uploaded evidence CSV is empty.');
        return;
      }

      const headers = rows[0].map((cell) => normalizeHeader(cell));

      const agentIndex = findHeaderIndex(headers, ['agent']);
      const ticketIdIndex = findHeaderIndex(headers, ['ticketid', 'externalcaseid', 'caseid']);
      const caseTypeIndex = findHeaderIndex(headers, ['casetype', 'type']);
      const caseDateIndex = findHeaderIndex(headers, ['ticketdate', 'casedate', 'date']);
      const customerMessageIndex = findHeaderIndex(headers, [
        'customermessage',
        'tickettext',
        'customertext',
        'conversation',
      ]);
      const agentReplyIndex = findHeaderIndex(headers, ['agentreply', 'agentreplytext', 'reply']);
      const internalNotesIndex = findHeaderIndex(headers, ['internalnotes', 'notes']);

      if (
        agentIndex === -1 ||
        ticketIdIndex === -1 ||
        customerMessageIndex === -1 ||
        agentReplyIndex === -1
      ) {
        setErrorMessage(
          'This CSV must include Agent, Ticket ID, Customer Message, and Agent Reply columns.'
        );
        return;
      }

      const nextPrepared: PreparedEvidenceRow[] = [];
      const nextSkipped: SkippedEvidenceRow[] = [];

      rows.slice(1).forEach((cells, rowIndex) => {
        const rowNumber = rowIndex + 2;
        const rawAgent = normalizeText(cells[agentIndex]);
        const externalCaseId = normalizeText(cells[ticketIdIndex]);
        const caseType = caseTypeIndex >= 0 ? normalizeText(cells[caseTypeIndex]) : '';
        const caseDate = caseDateIndex >= 0 ? normalizeText(cells[caseDateIndex]).slice(0, 10) : '';
        const customerMessage = normalizeText(cells[customerMessageIndex]);
        const agentReplyText = normalizeText(cells[agentReplyIndex]);
        const internalNotes =
          internalNotesIndex >= 0 ? normalizeText(cells[internalNotesIndex]) : '';
        ...
        if (!externalCaseId) {
          nextSkipped.push({
            rowNumber,
            agentLabel: rawAgent,
            reason: 'Missing Ticket ID / External Case ID.',
          });
          return;
        }

        if (!customerMessage && !agentReplyText) {
          nextSkipped.push({
            rowNumber,
            agentLabel: rawAgent,
            reason: 'Customer Message and Agent Reply are both empty.',
          });
          return;
        }
        ...
        nextPrepared.push({
          ...
          case_type: caseType,
          case_date: caseDate,
          ticket_text: customerMessage,
          agent_reply_text: agentReplyText,
      ...
      if (nextPrepared.length === 0) {
        setErrorMessage('No importable evidence rows were found in this CSV.');
      } else {
        setSuccessMessage(
          `${nextPrepared.length} evidence row(s) are ready to import. ${nextSkipped.length} row(s) will be skipped.`
        );
      }
    } catch (error) {
      setErrorMessage(
        error instanceof Error ? error.message : 'Could not parse the ticket evidence CSV.'
      );
    } finally {
  ...
  async function handleSaveCsv() {
    ...
    if (preparedCsvRows.length === 0) {
      setErrorMessage('Please upload a valid Ticket Evidence CSV first.');
      return;
    }
    ...
    try {
      ...
      const payload = preparedCsvRows.map((row) => ({
        ...
        ticket_text: row.ticket_text || null,
        agent_reply_text: row.agent_reply_text || null,
        internal_notes: row.internal_notes || null,
        created_by_user_id: currentUserId,
        metadata_json: {
          entry_method: 'csv',
          matched_agent_label: row.matchedAgentLabel,
      ...
      for (let start = 0; start < payload.length; start += chunkSize) {
        ...
        const { error } = await supabase.from('qa_evidence_cases').insert(chunk);
      ...
      setCsvFileName('');
      setSuccessMessage(
        `${payload.length} ticket evidence row(s) imported successfully.`
      );
    } catch (error) {
      setErrorMessage(
        error instanceof Error ? error.message : 'Could not save the ticket evidence rows.'
      );
    } finally {
      setCsvSaving(false);
    }
  ...
  const selectedAgent =
    agentProfiles.find((profile) => profile.id === draft.selectedAgentProfileId) || null;

  const previewCsvRows = useMemo(() => preparedCsvRows.slice(0, 10), [preparedCsvRows]);
  const previewSkippedCsvRows = useMemo(() => skippedCsvRows.slice(0, 12), [skippedCsvRows]);
  ...
  async function handleSaveManual() {
    ...
    if (!draft.externalCaseId.trim()) {
      setErrorMessage('Please fill Ticket ID / External Case ID.');
      return;
    }

    if (!draft.caseDate) {
      setErrorMessage('Please fill Ticket Date.');
      return;
    }

    if (!draft.customerMessage.trim() && !draft.agentReplyText.trim()) {
      setErrorMessage('Please provide Customer Message or Agent Reply.');
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase.from('qa_evidence_cases').insert({
        source_type: 'ticket',
        team: TEAM_NAME,
        external_case_id: draft.externalCaseId.trim(),
        agent_id: selectedAgent.agent_id,
        agent_name: selectedAgent.agent_name,
        case_type: draft.caseType.trim() || null,
        case_date: draft.caseDate,
        ticket_text: draft.customerMessage.trim() || null,
        agent_reply_text: draft.agentReplyText.trim() || null,
        internal_notes: draft.internalNotes.trim() || null,
        created_by_user_id: currentUserId,
        metadata_json: {
          entry_method: 'manual',
          matched_agent_label: getAgentLabel(selectedAgent),
        },
      });
      ...
      resetForm();
      setSuccessMessage('Ticket evidence saved successfully. Ready for AI evaluation in the next phase.');
    } catch (error) {
      setErrorMessage(
        error instanceof Error ? error.message : 'Could not save the ticket evidence record.'
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div style={{ color: 'var(--da-page-text, #e5eefb)' }}>
      <div style={pageHeaderStyle}>
        <div>
          <div style={sectionEyebrow}>AI Evidence Intake</div>
          <h2 style={{ margin: 0, fontSize: '30px' }}>Ticket Evidence Upload</h2>
          <p style={{ margin: '10px 0 0 0', color: 'var(--da-subtle-text, #94a3b8)' }}>
            Save real ticket conversations for future AI-assisted evaluation without changing the quantity upload page.
          </p>
        </div>
      ...
      <div style={highlightPanelStyle}>
        <div style={highlightHeaderStyle}>Phase 1 Scope</div>
        <p style={highlightTextStyle}>
          This page is for real ticket evidence only: ticket text, agent reply, case metadata, and notes.
          AI drafting will use these saved rows in the next step.
        </p>
      </div>
      ...
      <div style={csvPanelStyle}>
        <div style={csvHeaderStyle}>
          <div>
            ...
            <p style={csvSubtextStyle}>
              Upload ticket conversations in bulk. Required columns: Agent, Ticket ID, Customer Message, Agent Reply.
              Optional columns: Case Type, Ticket Date, Internal Notes.
            </p>
          </div>
        </div>
        ...
        <div style={formGridStyle}>
          <div style={wideFieldStyle}>
            <label style={labelStyle}>Ticket Evidence CSV File</label>
            <input
              type="file"
              accept=".csv,text/csv"
              onChange={(event) =>
            ...
            <div style={helperTextStyle}>
              Example headers: Agent, Ticket ID, Case Type, Ticket Date, Customer Message, Agent Reply, Internal Notes.
            </div>
          </div>
        ...
        <div style={actionRowStyle}>
          <button
            ...
            style={primaryButton}
          >
            {csvSaving ? 'Saving Evidence...' : 'Import Ticket Evidence CSV'}
          </button>
        ...
        {previewCsvRows.length > 0 ? (
          <div style={{ marginTop: '22px' }}>
            <div style={infoCardTitleStyle}>Evidence Preview</div>
            <div style={tableWrapStyle}>
              <div style={tableStyle}>
                <div style={{ ...tableRowStyle, ...tableHeaderRowStyle }}>
                  <div style={csvCellRowStyle}>Row</div>
                  <div style={csvCellAgentStyle}>Agent / Ticket</div>
                  <div style={csvCellCountStyle}>Case Type / Date</div>
                </div>

                {previewCsvRows.map((row) => (
                  <div key={`${row.rowNumber}-${row.external_case_id}`} style={entryStyle}>
                    <div style={tableRowStyle}>
                      ...
                      <div style={csvCellAgentStyle}>
                        ...
                        <div style={secondaryCellTextStyle}>
                          Ticket: {row.external_case_id}
                        </div>
                      </div>
                      <div style={csvCellCountStyle}>
                        <div style={primaryCellTextStyle}>{row.case_type || '-'}</div>
                        <div style={secondaryCellTextStyle}>{row.case_date || '-'}</div>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <div style={panelStyle}>
        <div style={formGridStyle}>
          <div style={wideFieldStyle}>
            <div style={infoCardTitleStyle}>Manual Single Ticket</div>
                ...
                <div style={pickerMenuStyle}>
                  ...
                  <div style={pickerListStyle}>
                    {loadingAgents ? (
                      ...
                      <div style={pickerInfoStyle}>No agents found for Tickets</div>
                    ) : (
          ...
          <div>
            <label style={labelStyle}>Ticket ID / External Case ID</label>
            <input
              type="text"
              ...
              onChange={(event) =>
                setDraft((prev) => ({
                  ...prev,
                  externalCaseId: event.target.value,
                }))
              }
              style={fieldStyle}
              placeholder="Enter ticket or external case id"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              style={fieldStyle}
              placeholder="Warranty, Refund, Missing, Exchange..."
            />
          </div>
          ...
          <div style={wideFieldStyle}>
            ...
            <textarea
              ...
              rows={6}
              style={fieldStyle}
              placeholder="Paste the customer message or full ticket body"
            />
          </div>

          <div style={wideFieldStyle}>
            <label style={labelStyle}>Agent Reply</label>
            <textarea
              value={draft.agentReplyText}
              onChange={(event) =>
                setDraft((prev) => ({
                  ...prev,
                  agentReplyText: event.target.value,
                }))
              }
              rows={6}
              style={fieldStyle}
              placeholder="Paste the agent reply that will later be evaluated by AI"
            />
          </div>
          ...
          <div style={wideFieldStyle}>
            ...
            <textarea
              ...
              rows={4}
              style={fieldStyle}
              placeholder="Optional case notes, context, or QA notes"
            />
          </div>
        </div>
      </div>
      ...
      <div style={actionRowStyle}>
        <button onClick={() => void handleSaveManual()} disabled={saving} style={primaryButton}>
          {saving ? 'Saving Evidence...' : 'Save Ticket Evidence'}
        </button>
        ...
        <button
          ...
          onClick={() => {
            resetForm();
            setErrorMessage('');
            setSuccessMessage('Ticket evidence draft cleared.');
          }}
          disabled={saving}
          style={secondaryButton}
        >
...
const highlightPanelStyle = {
  ...
  marginBottom: '20px',
  background:
    'linear-gradient(180deg, rgba(37, 99, 235, 0.16) 0%, var(--da-card-bg, rgba(15, 23, 42, 0.56)) 100%)',
};
...
export default TicketEvidenceUploadSupabase;
